...
- **Credential**: `${input:chat.lm.secret...}` của VS Code không resolve ngoài VS Code. Tên provider suy ra key env: `Hollow` → `HOLLOW_API_KEY`. Hoặc `apiKey` catalog dạng `${env:PROVIDER_API_KEY}`. API key literal bị từ chối. Copilot setup mặc định tạo input ẩn mới của VS Code, map sang key env an toàn; không đọc kho secret sẵn. Launch khác hoặc `--credentials env`: cấp credential trong môi trường orchestrator trước khi launch/resume worker.
